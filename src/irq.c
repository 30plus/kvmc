...
static bool check_for_irq_routing(struct kvm *kvm)
{
	...
	if (has_irq_routing == 0) { // TODO: Sure
...
static int irq__update_msix_routes(struct kvm *kvm, struct kvm_irq_routing_entry *entry)
{
...
int irq__common_add_irqfd(struct kvm *kvm, unsigned int gsi, int trigger_fd, int resample_fd)
{

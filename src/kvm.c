...
int kvm__recommended_cpus(struct kvm *kvm)
{
	...
	if (ret <= 0)
		/* api.txt states that if KVM_CAP_NR_VCPUS does not exist, assume 4. */
		return 4;
...
int kvm__init(struct kvm *kvm)
{
	...
	if (kvm->sys_fd < 0) {
		if (errno == ENOENT)
			...
			pr_err("'%s' KVM driver not available.\n  # (If the KVM module is loaded then 'dmesg' may"
				" offer further clues about the failure.)", kvm->cfg.dev);
		else
...
static bool initrd_check(int fd)
{
	...
	return (id[0] == GZIP_ID1 && id[1] == GZIP_ID2) || !memcmp(id, CPIO_MAGIC, 4);
}

...
/* This is within our PCI gap - in an unused area. Note this is a PCI
 * *bus address*, is used to assign BARs etc.! (That's why it can still 32bit
 * even with 64bit guests-- 64bit PCI isn't currently supported.)
 */
...
void pci__config_wr(struct kvm *kvm, union pci_config_address addr, void *data, int size)
{
	...
	/* If the kernel masks the BAR it would expect to find the size of the BAR
	 * there next time it reads from it. When the kernel got the size it would

...
static int vfio_device_pci_parser(char *arg, struct vfio_device_params *dev)
{
...
int vfio_device_parser(struct kvm *kvm, const char *arg, int unset)
{
	int ret = -EINVAL;
	static int idx = 0;
	...
	ret = vfio_device_pci_parser(cur, dev);
	//else
	//	ret = -EINVAL;

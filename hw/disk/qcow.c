...
static int cache_table(struct qcow *q, struct qcow_l2_table *c)
{
	...
	if (l1t->nr_cached == MAX_CACHE_NODES) {
		/*
		 * The node at the head of the list is least recently used node.
		 * Remove it from the list and replaced with a new node.
		 */
...
/*
 * Allocate clusters according to the size. Find a postion that can satisfy the size.
 * free_clust_idx is initialized to zero and Record last position.
 */
static u64 qcow_alloc_clusters(struct qcow *q, u64 size, int update_ref)
{
	struct qcow_header *header = q->header;
...
/*
 * Get l2 table. If the table has been copied, read table directly. If the table exists, allocate a new cluster
 * and copy the table to the new cluster.
 */
...
/* If the cluster has been copied, write data directly. If not, read the original data and
 * write it to the new cluster with modification. */

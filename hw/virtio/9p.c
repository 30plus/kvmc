...
int virtio_9p_rootdir_parser(const char *arg, struct kvm *kvm)
{
	char *tag_name;
	char tmp[PATH_MAX];

	/* 9p dir can be of the form dirname,tag_name or just dirname. In the later case we use the default tag name */
	tag_name = strstr(arg, ",");
	if (tag_name) {
		*tag_name = '\0';
		tag_name++;
...
int virtio_9p_img_name_parser(const char *arg, struct kvm *kvm)
{
	char path[PATH_MAX];
	struct stat st;
...
int virtio_9p__init(struct kvm *kvm)
{
	...
	list_for_each_entry(p9dev, &devs, list) {
		virtio_init(kvm, p9dev, &p9dev->vdev, &p9_dev_virtio_ops, VIRTIO_DEFAULT_TRANS(kvm),
			PCI_DEVICE_ID_VIRTIO_9P, VIRTIO_ID_9P, PCI_CLASS_9P);
	}
